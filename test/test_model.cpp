...
Point2D transformToModel(Point p) {
  return {transform(0, 127, margin, x_max, p.x),
          transform(0, 127, margin, y_max, p.y)};
}

Point transformToLCD(Point2D p) {
  return {(uint8_t)transform(margin, x_max, 0, 63, p.x),
          (uint8_t)transform(margin, y_max, 0, 63, p.y)};
}
...
int main() {
  ...
  for (const auto &point : points) {
    auto p_model = transformToModel({point.x, point.y});
